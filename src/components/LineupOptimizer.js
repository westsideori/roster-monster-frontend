...
const LineupOptimizer = ({playerPredictions, currentUser}) => {
  ...
  const playersWithFantasyPoints = filteredPlayerPredictions.map((player) => {
    return ({...player, 
      fantasyPoints: (player.Games * selectedRoster.score_setting['games_played']) + (player.FieldGoalsAttempted * selectedRoster.score_setting['field_goals_attempted']) + (player.FieldGoalsMade * selectedRoster.score_setting['field_goals_made']) + (player.ThreePointersAttempted * selectedRoster.score_setting['threes_attempted']) + (player.ThreePointersMade * selectedRoster.score_setting['threes_made']) + (player.FreeThrowsAttempted * selectedRoster.score_setting['free_throws_attempted']) + (player.FreeThrowsMade * selectedRoster.score_setting['free_throws_made']) + (player.Points * selectedRoster.score_setting['points']) + (player.Rebounds * selectedRoster.score_setting['rebounds']) + (player.Assists * selectedRoster.score_setting['assists']) + (player.Steals * selectedRoster.score_setting['steals']) + (player.BlockedShots * selectedRoster.score_setting['blocks']) + (player.Turnovers * selectedRoster.score_setting['turnovers'])})
  })
  ...
  const pointGuardsPoints = pointGuards.sort((a, b) => b.fantasyPoints - a.fantasyPoints)
  const shootingGuardsPoints = shootingGuards.sort((a, b) => b.fantasyPoints - a.fantasyPoints)
  const forwardsPoints = forwards.sort((a, b) => b.fantasyPoints - a.fantasyPoints)
  const smallForwardsPoints = smallForwards.sort((a, b) => b.fantasyPoints - a.fantasyPoints)
  const powerForwardsPoints = powerForwards.sort((a, b) => b.fantasyPoints - a.fantasyPoints)
  const centersPoints = centers.sort((a, b) => b.fantasyPoints - a.fantasyPoints)
  ...
  const optimizeMyLineup = () => {
    ...
    if (lineupForm.pointGuards > 0) {
      const topPointGuardsPoints = pointGuardsPoints.slice(0, lineupForm.pointGuards)
      preLineup = preLineup.concat(topPointGuardsPoints)
    } 
    if (lineupForm.shootingGuards > 0) {
      const topShootingGuardsPoints = shootingGuardsPoints.slice(0, lineupForm.shootingGuards)
    ...
    if (lineupForm.smallForwards > 0) {
      const topSmallForwardsPoints = smallForwardsPoints.slice(0, lineupForm.smallForwards)
      preLineup = preLineup.concat(topSmallForwardsPoints)
    } 
    if (lineupForm.powerForwards > 0) {
      const topPowerForwardsPoints = powerForwardsPoints.slice(0, lineupForm.powerForwards)
